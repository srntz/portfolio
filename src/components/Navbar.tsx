...
export default function Navbar({animate = true, delay}:{animate?: boolean, delay: number}) {
  const [exitSequenceTriggered, setExitSequenceTriggered] = useState(false);

    if(animate) {
      return (
        <AnimatePresence>
        {!exitSequenceTriggered &&
          <motion.div id="navbar" exit={{marginTop: 20, opacity: 0}} className={`${containerClasses} ${containerChildClasses}`}>
            <motion.p onClick={() => setExitSequenceTriggered(true)} initial={{opacity: 0, marginBottom: -20}} whileHover={{marginTop: -10, transition:{duration: 0.3, type: "spring", bounce: 0}}} animate={{opacity: 0.8, marginBottom: 0, transition:{delay: 3.5}}}>About me</motion.p>
